    ...
    <section id="services" className="relative overflow-clip bg-[var(--ink-soft)] py-24 sm:py-32">
      ...
      <div className="site-container relative">
        <motion.header variants={reveal} initial={reduceMotion ? false : "hidden"} whileInView="visible" viewport={{ once: true, amount: 0.3 }} className="mb-14 grid gap-8 lg:mb-20 lg:grid-cols-[1fr_25rem] lg:items-end">
          <div><p className="eyebrow mb-6 text-[var(--cyan)]">02 / What we do</p><h2 className="display-type max-w-4xl text-[clamp(2.35rem,5.4vw,4.9rem)] leading-[.92] text-white">Growth is a<span className="text-gradient block">connected system.</span></h2></div>
          <p className="border-l border-[var(--cyan)] pl-5 text-[15px] leading-7 text-[var(--muted)] sm:text-[17px]">Strategy, creative, technology, and optimization working together—never as disconnected deliverables.</p>
        </motion.header>
              ...
              >
                ...
                <div className="grid h-full gap-8 p-6 sm:p-8 lg:min-h-[34rem] lg:grid-cols-[.72fr_1.28fr] lg:items-center lg:p-11">
                  <div>
                    ...
                    <h3 className="display-type mt-5 max-w-xl text-balance text-[clamp(1.75rem,2.9vw,3rem)] leading-[.98] text-white">{service.title}</h3>
                  </div>
                  <div className="lg:border-l lg:border-white/10 lg:pl-10">
                    <p className="max-w-2xl text-pretty text-[15px] leading-7 text-white/72 sm:text-[17px]">{service.description}</p>
                    <p className="mt-5 max-w-2xl text-[13px] leading-7 text-[var(--muted)] sm:text-[14px]">{service.detail}</p>
    ...
    <section id="process" className="relative overflow-hidden bg-[var(--paper)] py-24 text-[var(--ink)] sm:py-32">
      ...
      <div className="site-container relative">
        <motion.header variants={reveal} initial={reduceMotion ? false : "hidden"} whileInView="visible" viewport={{ once: true, amount: 0.3 }} className="mb-14 grid gap-8 lg:mb-20 lg:grid-cols-[1fr_26rem] lg:items-end">
          <div><p className="eyebrow mb-6 text-[#5f7820]">03 / The 6-D framework</p><h2 className="display-type max-w-5xl text-[clamp(2.3rem,4.6vw,4.3rem)] leading-[.94]">From uncertainty<span className="text-gradient-dark block">to momentum.</span></h2></div>
          <p className="border-l border-black/25 pl-5 text-[15px] leading-7 text-black/58 sm:text-[17px]">A clear operating rhythm that removes guesswork, aligns every decision, and keeps performance visible.</p>
        </motion.header>
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {PROCESS.map((step, index) => (
            <motion.article key={step.number} initial={reduceMotion ? false : { opacity: 0, y: 32 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.24 }} transition={{ duration: 0.65, delay: (index % 3) * 0.08, ease: [0.16, 1, 0.3, 1] }} className="group relative min-h-[17.5rem] overflow-hidden rounded-[1.75rem] border border-black/8 bg-white/65 p-6 shadow-[0_22px_60px_rgba(6,8,11,.08)] transition duration-500 hover:-translate-y-2 hover:bg-white sm:p-8">
              ...
              <div className="relative flex h-full flex-col"><div className="flex items-center justify-between"><span className="display-type text-[2.35rem] text-[#617b20] sm:text-[2.85rem]">{step.number}.</span><ArrowUpRight className="size-5 text-black/30 transition group-hover:rotate-45 group-hover:text-black" /></div><div className="mt-auto pt-10"><h3 className="display-type text-[1.25rem] sm:text-[1.55rem]">{step.title}</h3><p className="mt-3 text-[13px] leading-6 text-black/58 sm:text-[14px]">{step.description}</p></div></div>
            </motion.article>
          ))}
        </div>
      </div>
    ...
    <section id="results" className="relative overflow-hidden bg-[var(--ink)] py-24 sm:py-32">
      ...
      <div className="site-container relative">
        <motion.header variants={reveal} initial={reduceMotion ? false : "hidden"} whileInView="visible" viewport={{ once: true, amount: 0.3 }} className="mb-14 grid gap-8 lg:mb-20 lg:grid-cols-[1fr_24rem] lg:items-end">
          <div><p className="eyebrow mb-6 text-[#f7c873]">04 / Client signal</p><h2 className="display-type max-w-5xl text-[clamp(2.35rem,5.4vw,4.9rem)] leading-[.92] text-white">Built for real<span className="text-gradient block">business impact.</span></h2></div>
          <div className="rounded-3xl border border-white/10 bg-white/[.035] p-6"><div className="flex items-center gap-3 text-[var(--acid)]"><ShieldCheck className="size-5" /><span className="mono-type text-[10px] font-bold uppercase tracking-[.16em]">Execution over theatre</span></div><p className="mt-4 text-[13px] leading-7 text-[var(--muted)]">Systems that improve visibility, speed, control, and long-term scalability.</p></div>
        </motion.header>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="relative flex w-full flex-col">
                        ...
                        <blockquote className="display-type mt-10 text-balance text-[clamp(1.18rem,1.55vw,1.55rem)] leading-[1.12] text-white">
                          “{item.quote}”
                        </blockquote>
